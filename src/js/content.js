 ...
 *
 * Note that this content script will not get executed when the
 * user navigates to the feed via navbar. See the comment below
 * for more details.
 */
...
/*
 * Activate the extension when user navigate back to the feed
 * from a different page.
 *
 * Navigation via navbar (or any other navigation means except
 * address bar) in LinkedIn would dynamically load and replace
 * HTML contents instead of fetching a webpage.
 *
...
async function activate() {
  ...
  infiniteScrollObserver = new MutationObserver(mutations => {
    for (let mutation of mutations) {
      ...
      if (target.nodeType === Node.ELEMENT_NODE
          ...
          && mutation.addedNodes[0].getAttribute("data-urn").includes("urn:li:activity")) {
   ...
   * via infinite scroll.
   */
  infiniteScrollObserver.observe(feedContainer, { childList: true, subtree: true });
  ...
  document.querySelector(Selector.FILTER_PANEL_APPLY_BTN).onclick = async () => {
    /*
     * When storing user preferences, only excluded activity types
     * (those with unchecked checkboxes) are stored.
     */
    let filters = Array.from(document.querySelectorAll(Selector.FILTER_PANEL_CHECKBOXES))
   ...
   * dynamically via infinite scroll, and the filters are too aggressive,
   * all newly fetched activities will be removed. Yet, the infinite
   * scroll loader requires more scroll events to fetch new activities.
   *
   * TODO use Intersection Observer instead to reduce the overhead.
   */
  scrollEventInterval = setInterval(() => {
    if (document.body.offsetHeight - (window.innerHeight + window.pageYOffset) <= 975) {
