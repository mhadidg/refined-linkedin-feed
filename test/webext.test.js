 ...
 * account username (IN_USERNAME) and password (IN_PASSWORD) must
 * be provided via .env file or some other method.
 */
...
describe("User preferences", () => {
  ...
  test("are stored on apply", async () => {
    ...
    await expect(page).toMatchElement("#filter-panel", { timeout: 5000 });
    await expect(page).toMatchElement("input#promoted_post.filter:not(:checked)", { timeout: 500 });
  });
